...
export async function ingestPdf(client: SupabaseClient, config: EmbeddingConfig, buffer: Uint8Array, source: string, job?: { id: string; worker: string }): Promise<boolean> {
  const chunks = chunkText(await extractPdfText(buffer));
  if (!chunks.length) throw new Error("PDF tidak memiliki teks yang dapat dibaca. Gunakan PDF teks, bukan hasil scan.");
